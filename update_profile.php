<?php 
	...
	$update_nm = mysqli_query($conn, "UPDATE user_form SET name = '$update_name', email = '$update_email' WHERE user_id = '$user_id' ");
		if($update_nm){
	...
	if(!empty($image)){
		if($image_size > 2000000){
			...
			$update_img = mysqli_query($conn, "UPDATE user_form SET img = '$image' WHERE user_id = '$user_id' ");
	...
	if(!empty($main_pass) || !empty($main_pass) || !empty($confirm_pass)){
		if($old_pass != $main_pass){
			$alert[] = "old password not matched!";
		}elseif($new_pass != $confirm_pass){
			$alert[] = "confirm password not matched!";
		}else {
			$update_pass = mysqli_query($conn, "UPDATE user_form SET password = '$confirm_pass' WHERE user_id = '$user_id' ");
...
<body>
	<div class="update-profile">
		...
		<div class="flex">
			...
			<input type="password" name="old_pass" class="box password-input" required />
			<i class="fas fa-eye toggle-password" onclick="togglePassword('old_pass')"></i>
			</div>

			<span>new password :</span>
			<div class="password-container">
			<input type="password" name="new_pass" class="box password-input" required />
			<i class="fas fa-eye toggle-password" onclick="togglePassword('new_pass')"></i>
			</div>

			<span>confirm password :</span>
			<div class="password-container">
			<input type="password" name="confirm_pass" class="box password-input" required />
